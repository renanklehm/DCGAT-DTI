"""Bounded Neo4j writer used by bulk inference."""
...
class Neo4jPredictionWriter:
    def __init__(self, config_path: str | Path) -> None:
        ...
        kwargs = {key: config[key] for key in ("encrypted", "connection_timeout") if key in config}
        self.driver = GraphDatabase.driver(config["uri"], auth=(config["username"], config["password"]), **kwargs)
